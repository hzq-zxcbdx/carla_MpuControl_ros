...
def main():
    ...
    try:
        rospy.init_node("carla_listen")
        sub = rospy.Subscriber("mpu6050_data",mpu6050msg,mpu_control,queue_size=10)
        mpu_thread = threading.Thread(target = mpu_spin)
        mpu_thread.start()
        ...
        while(1):
            ...
            if(pitch > -10):
                ...
                if(roll > 10):
                    print("turn left")
                    vehicle.apply_control(carla.VehicleControl(throttle=0.2, steer = -(roll)*0.02))
                elif(roll < -10):
                    print("turn right")
                    vehicle.apply_control(carla.VehicleControl(throttle=0.2, steer = -(roll)*0.02))
                else:
            ...
            sepctator.set_transform(carla.Transform(transform.location + carla.Location(z=30),carla.Rotation(pitch=-90)))
            time.sleep(0.05)
